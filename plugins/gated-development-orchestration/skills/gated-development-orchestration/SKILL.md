...
Governance Triage loads the workflow core first and classifies the BLOCKED return; it is not a sixth round and cannot PASS. GDO 4.6 preserves Human TAKEOVER, correction/evidence economy, delta-oriented lifecycle comments, targeted development checks, and Discovery Probe semantics, and adds opt-in Evidence Admission plus stable proof-ledger contracts before fresh Evidence dispatch. Triage may identify that a probe is needed but cannot own/issue it; route to Discovery first. Probe and targeted-check results are feedback only and never Evidence.
