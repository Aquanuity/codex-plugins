...
Opt-in READY handoffs marked `gated-development:evidence-admission:v1` bind candidate/branch, recipe and `gdo-proof-ledger/v1`, then pass a non-round runner gate. Only `ADMITTED` launches fresh Evidence; `NOT_READY | TRIAGE_REQUIRED` do not. Admission is never proof; deterministic failure beats optional AI; unmarked handoffs stay legacy.
